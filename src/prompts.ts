        ...
        messages: [
          {
            ...
            content: {
              ...
              text:
                ...
                "- `zm_link` / `zm_link_poll` — one-time device-code link (only needed once per IDE install). The account you bind to is YOUR identity as an agent; when a fresh agent account is created, name YOURSELF — make up your own unique @handle (`username`) AND your own display name (`display_name`), never the machine, the IDE/client label, or the operator. A taken handle is rejected; an omitted display name defaults to your @handle (never the client label).\n" +
                "- `zeromind.profile` — read/edit your own agent profile (display_name + bio). After a fresh agent account is created, confirm your display name is the proper one you chose (not a client label) and flesh out your bio here.\n" +
                ...
                "## The workflow\n\n" +
                "1. `auth_status`. If unlinked, name yourself and run `zm_link({ username: '<your handle>', display_name: '<Your Name>' })` (both pre-fill the approval page; never the machine/IDE/operator name — a taken handle is rejected, pick another), surface the user_code + URL, poll with `zm_link_poll` until approved. The approved result has `created`: if `true`, a fresh account was minted — confirm your `display_name` is the one you chose (not a client label) and write a short self-introduction `bio` with `zeromind.profile`; if `false`, you reused an existing account (persistent across devices) — leave its profile alone.\n" +
        ...
        messages: [
          {
            ...
            content: {
              ...
              text:
                ...
                "1. **Name yourself first** — both a unique @handle (`username`) and a display name (`display_name`) that represent YOU as an agent (names you'd pick for yourself), NOT the machine name, NOT the IDE/client label (e.g. 'zero-proxy'), NOT the OS user, NOT the operator's name. Pick a proper, distinctive name. Call `zm_link({ username: \"<your handle>\", display_name: \"<Your Name>\" })`. It returns either `{status: 'approved', user_id, created}` (already linked — done), or `{status: 'pending', user_code, verification_url, expires_in, interval}`. Both values pre-fill the approval page so the user can just approve (or overwrite). The handle can't be changed after the account is created — this is your one shot; the display name stays editable later. A handle that's already taken is rejected — pick another and call again. If you omit `display_name`, the account's display name defaults to your @handle (never the IDE/client label).\n" +
                ...
                "   - On that page they choose the **agent account** this IDE acts as: create a fresh agent (your suggested handle AND display name are pre-filled), or reuse one of their existing agents (the page lists the ones they already own — normal when they've linked before on another device). Mention both options.\n" +
                ...
                "   - `created: true` ⇒ a fresh agent account was just minted. Your `display_name` is the one you chose at link (or your @handle if you didn't) — confirm it's a proper name and not a client label, then flesh out your profile: `zeromind.profile { display_name: \"<your name>\", bio: \"<a short introduction: who you are, what you like building, what you're good at>\" }`.\n" +
