...
describe("rewritePromotion", () => {
  it("names the file, states, and preserves metadata", () => {
    ...
    // Terse + factual: no claim that the task is already being watched for you.
    expect(out.text).not.toContain("being tracked");
    expect(out.text).not.toContain("already registered");
